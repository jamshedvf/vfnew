...
?>
		...
		//  $sliders = get_group('portfolio_images');
//var_dump($sliders);

	 // echo $i;   
      //echo $slider['portfolio_images_image'][$i]['original'];
	  //$i=$i+1;
	   $value  = get_field ('portfolio_images_image') ;
       foreach ( $value  as  $my_val ) { 
?>
     
     
         
            <li><img src="<?php echo $my_val['original'];?>" alt="img"></li>
            <?php 
	...
	if(sizeof($term_list)>0 && $tl<$totalsize-1)
	{
	 ...
	 ?>
          ...
          <li><b> URL :</b><a href="<?php echo get_post_meta( get_the_ID(), 'portfolio_info_url', true );?>"> <?php echo get_post_meta( get_the_ID(), 'portfolio_info_url', true );?></a></li>
        </ul>
